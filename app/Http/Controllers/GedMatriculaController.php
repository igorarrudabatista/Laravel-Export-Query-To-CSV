...
class GedMatriculaController extends Controller
{
    public function exportCsv(Request $request)
    {
       ...
       cast(CASE WHEN L.GerLotIdINEP = 51036550 THEN 51027 ELSE cast(org.GerOrgRegCodEdu as varchar) END as varchar) AS sre, /*regional*/
       'Estadual' as tipo_dependencia,
       COALESCE(L.GerLotIdINEP,99999999) AS inep_escola,
       l.GerLotNom AS nome_escola,
       pl.GerPrmEndLogNom as endereco_escola,
       cast(rtrim(pl.GerPrmLotNmrLog) as nvarchar) as numero_escola,
       '' as complemento_escola,
       pl.GerPrmBaiNom as bairro_escola,
       COALESCE(C.GerCidId,-1) AS codibge7_escola,
       cast(rtrim(c.GerCidNom) as nvarchar) AS municipio_escola,
       'MT' as estado_escola,
       ...
       '' AS submodalidade,
       COALESCE(rtrim(eee.GerEtpEnsNom),rtrim(ee.GerEtapaEnsinoDsc)) AS serie,
       ...
       cast(rtrim(c2.GerCidNom) as nvarchar) as municipio_aluno,
       ...
       cast(rtrim(P.GerPesNmrLog) as nvarchar) AS numero_aluno,
       ...
       CASE WHEN EXISTS (SELECT NE.GedAluNecEduEspCod 
                         ...
                         WHERE NE.GedAluCod = M.GedAluCod AND NE.GedAluNecEduEspCod = 6 /*DEFICIÊNCIA FÍSICA*/) THEN 'S' ELSE 'N' END AS mobilidade_reduzida,
       CASE WHEN EXISTS (SELECT NE.GedAluNecEduEspCod 
                         FROM GED.TBGEDNECESSIDADESESPECIAIS NE
                         WHERE NE.GedAluCod = M.GedAluCod AND NE.GedAluNecEduEspCod = 6 /*DEFICIÊNCIA FÍSICA*/) THEN 'S' ELSE 'N' END AS veiculo_especial,
       CASE WHEN M.GedMatUtiTraEsc = 1 THEN 'S' ELSE 'N' END AS frota_propria,
       '' AS utiliza_transporte_rural,
       ...
       /*AND COALESCE(MR.GerMatRegFlgOpt,0) = 0   
       AND COALESCE(MR.GerMatRegFlgOptIndependente,0) = 0  */
       AND m.GedMatDep = 0 /*exclui matrículas de dependências*/
       ");  
